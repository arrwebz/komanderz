<table id="datatables" class="table border table-striped table-bordered display text-nowrap" style="width: 100%">
    ...
    foreach ( $drd as $row ) {
        $i++;
        if($row['tipe_pinjaman'] == '5'){
            $show_tipepenarikan = '<span class="label label-default">Insidentil</span>';
        }elseif($row['tipe_pinjaman'] == '6'){
            $show_tipepenarikan = '<span class="label label-default">Super Telco</span>';
        }elseif($row['tipe_pinjaman'] == '7'){
            $show_tipepenarikan = '<span class="label label-default">Bank</span>';
        }else{
        ...
        <tr>
            ...
            <td style="color: #fa591d;">
                <strong><?php echo strrev(implode('.',str_split(strrev(strval($row['nominal_pinjaman'])),3)));?></strong>
            </td>
            <td>
            ...
            <td class="text-center">
                <a class="image-popup-vertical-fit" target="_blank" href="<?php echo $this->config->item('uploads_uri').'spbsp/'.$row['spbspid'].'/'.$row['bukti_transfer'];?>">
