<h2 class="title-segmen" style="margin-top:0px !important; margin-left:20px;"><?php echo $segmen;?> (Unpaid)</h2>
...
<?php if(count($drd) > 0):  ?>
	...
	<div id="chartSegmenBelumCair" style="width: 100%;height: 500px;"></div>
	...
	<table class="table table-responsive table-bordered">
		...
		echo '<tbody>';
			echo '<tr>';
				echo '<td width="20%"><i class="ti ti-square" style="color:#C20000"></i> '. $segmen .'</td>';
				if(is_numeric($key_januari)){
	...
	<script>
		...
		var chartBelumCair = am4core.create("chartSegmenBelumCair", am4charts.XYChart3D);
		chartBelumCair.data = <?php echo $drd_chart?>;

		// Modify chart's colors
		chartBelumCair.colors.list = [
		...
		// Create axes
		var categoryAxis = chartBelumCair.xAxes.push(new am4charts.CategoryAxis());
		...
		var valueAxis = chartBelumCair.yAxes.push(new am4charts.ValueAxis());
		...
		// Create series
		var series = chartBelumCair.series.push(new am4charts.ColumnSeries3D());
		...
		columnTemplate.adapter.add("fill", function(fill, target) {
			return chartBelumCair.colors.getIndex(target.dataItem.index);
		});
		...
		chartBelumCair.cursor = new am4charts.XYCursor();
		chartBelumCair.cursor.lineX.strokeOpacity = 0;
		chartBelumCair.cursor.lineY.strokeOpacity = 0;
	</script>
<?php else: ?>
	<div class="text-center">
