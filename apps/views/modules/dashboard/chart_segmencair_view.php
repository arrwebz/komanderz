<h2 class="title-segmen" style="margin-top:0px !important; margin-left:20px;"><?php echo $segmen;?> (Paid)</h2>
...
<?php if(count($drd) > 0):  ?>
	...
	<div id="chartSegmenCair" style="width: 100%;height: 500px;"></div>
	...
	<table class="table table-responsive table-bordered">
		...
		echo '<tbody>';
			echo '<tr>';
				echo '<td width="20%"><i class="ti ti-square" style="color:#2b3436"></i> '. $segmen .'</td>';
				if(is_numeric($key_januari)){
	...
	<script>
		...
		var chartCair = am4core.create("chartSegmenCair", am4charts.XYChart3D);
		chartCair.data = <?php echo $drd_chart?>;

		// Modify chart's colors
		chartCair.colors.list = [
			am4core.color("#2b3436"),
			am4core.color("#2b3436"),
			am4core.color("#2b3436"),
			am4core.color("#2b3436"),
			am4core.color("#2b3436"),
		...
		// Create axes
		var categoryAxis = chartCair.xAxes.push(new am4charts.CategoryAxis());
		...
		var valueAxis = chartCair.yAxes.push(new am4charts.ValueAxis());
		...
		// Create series
		var series = chartCair.series.push(new am4charts.ColumnSeries3D());
		...
		columnTemplate.adapter.add("fill", function(fill, target) {
			return chartCair.colors.getIndex(target.dataItem.index);
		});
		...
		chartCair.cursor = new am4charts.XYCursor();
		chartCair.cursor.lineX.strokeOpacity = 0;
		chartCair.cursor.lineY.strokeOpacity = 0;
	</script>
<?php else: ?>
	<div class="text-center">
