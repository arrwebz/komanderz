                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a class="text-muted text-decoration-none" href="<?php echo site_url('letter');?>">Contract & Purchase Order</a>
                        </li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="card bg-warning-subtle shadow-none">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center">
                        ...
                        <h6 class="mb-0 ms-3">Contract PKS</h6>
                    ...
                    <div class="d-flex align-items-center justify-content-between mt-4">
                        <h3 class="mb-0 fw-semibold fs-7 pks">Loading...</h3>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-xl-6">
            <div class="card bg-info-subtle shadow-none">
                <div class="card-body p-4">
                    <div class="d-flex align-items-center">
                        <div class="d-flex align-items-center justify-content-center">
                            <img src="<?php echo $this->config->item('images_uri');?>svgs/icon-connect.svg" width="50" height="50" class="mb-3" alt="">
                        </div>
                        <h6 class="mb-0 ms-3">PO Project</h6>
                        <div class="ms-auto text-primary d-flex align-items-center">
                            <button class="btn btn-light">Refresh</button>
                        </div>
                    </div>
                    <div class="d-flex align-items-center justify-content-between mt-4">
                        <h3 class="mb-0 fw-semibold fs-7 poprojek">Loading...</h3>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-4">
                            ...
                            <select name="type" class="form-control selectpicker" style="width: 100%; height: 36px">
								...
								<option value="PO">PO Project</option>
                            </select>
                        </div>
                        <div class="form-group mb-4">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select name="type" class="form-control selectpicker" data-width="100%">
                            ...
                            <option value="PKS">PKS</option>
                            <option value="PO">PO Projek</option>
                        </select>
                    </div>
                    <div class="form-group mb-3">
...
<script>
	$(function(){
        ...
        $(".refresh-nomor-surat").on("click", function () {
	        $(".pks").html("Loading...");
	        $(".poprojek").html("Loading...");

	        $.ajax({
		        type: "POST",
		        url: "<?php echo site_url('letterkontrak/nomorsurat')?>",
		        data: "",
		        success: function (data) {
			        var respon = JSON.parse(data);
			        if(respon['status'] == 'success'){
			        	setTimeout(function(){
					        $(".pks").html(respon['data']['pks']);
					        $(".poprojek").html(respon['data']['po']);
			        	}, 1000);
			        }else{
				        $(".pks").html('System not responding');
				        $(".poprojek").html('System not responding');
			        }
		        }
	        });
        });
		...
		$("#formSearch").on("submit", function () {
			...
			$.ajax({
			    type: "POST",
			    url: "<?php echo site_url('letterkontrak/listdata')?>",
			    data: dataPost,
			    success: function (data) {
			        var respon = JSON.parse(data);
			        if(respon['status'] == 'success'){
		...
		$("#formAddLetter").on('submit', function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('letterkontrak/ajaxadd')?>",
                data: dataPost,
                success: function (data) {
                    var respon = JSON.parse(data);
                    if(respon['status'] == 'success'){
        ...
        $(document).on("click", ".edit-row", function () {
	        ...
	        $.ajax({
	            type: "POST",
	            url: "<?php echo site_url('letterkontrak/ajaxdetail')?>",
		        data: 'id='+id,
	            success: function (data) {
	                var respon = JSON.parse(data);
	                if(respon['status'] == 'success'){
        ...
        $("#formEditLetter").on("submit", function () {
            ...
            $.ajax({
               type: "POST",
               url: "<?php echo site_url('letterkontrak/ajaxedit')?>",
               data: dataPost,
               success: function (data) {
                    var respon = JSON.parse(data);
                    if(respon['status'] == 'success'){
        ...
        $(document).on("click",".delete-row",function() {
	        ...
	        swal({
		        ...
		        if (willDelete) {
                    $.ajax({
                        url: "<?php echo site_url('letterkontrak/ajaxdelete');?>",
                        data: 'id=' + id,
                        type: 'POST',
                        success: function(data) {
	                        var respon = JSON.parse(data);
