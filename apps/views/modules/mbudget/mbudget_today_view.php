                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a class="text-muted text-decoration-none" href="<?php echo site_url('mbudget');?>">Budget</a>
                        </li>
                        <li class="breadcrumb-item" aria-current="page">Konfirmasi SPB MESRA</li>
                    </ol>
                </nav>
            </div>
            <div class="col-3">
                                ...
                                if ($rowcount != 0) {
                                    ...
                                    for ($row = 0; $row < $rowcount; $row++) {
                                        ...
                                        if ($rowtable[$row][0]['status'] == 0 ){
                                            echo '<td><small class="badge pull-right bg-red">X</small></td>';
                                        } else {
                                            echo '<td><small class="badge pull-right bg-green">OK</small></td>';
                                        }
                                        echo '</tr>';
                                    }
...
<script>
	$(function() {
		...
		$("#btnConfirm").click(function() {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('mbudget/ajaxconfirmbudget'); ?>",
				data: $('#formupdatebudget').serialize(),
				success: function(data) {
					/* play sound */
					swal({title: "Berhasil", text: "Konfirmasi data anggaran berhasil!", icon:
		...
		$("#btnUpdate").click(function() {
			$.ajax({
				type: "POST",
				url: "<?php echo site_url('mbudget/ajaxupdatebudget'); ?>",
				data: $('#formupdatebudget').serialize(),
				success: function(data) {
					/* play sound */
					swal({title: "Berhasil", text: "Perbarui data anggaran berhasil!", icon:
								"success", buttons: false, timer: 1500,}).then(function(){
								location.reload();
							}
					);
