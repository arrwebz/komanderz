...
class Preportnps extends CI_Controller
{
    ...
    public function index() {
		$data = [ 
			'drdnday' => json_encode($this->repmd->getreportnopesperday('PADI')),
			'drdnmonth' => json_encode($this->repmd->getreportnopespermonth('PADI')),
		...
		$data['content'] = $this->load->view('modules/preport/preport_nopes_view', $data, TRUE);
	...
	public function reportfilter() {
		...
		$data['content'] = $this->load->view('modules/preport/pfilter_nopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
    }
	...
	public function exportfilter(){
		...
		$strftitle = $segment.'/'.$fdate.'/'.$edate.'/PADI';
		$data = array( 'title' => 'Laporan Invoice Padi : '.$strftitle,
                'drd' => $this->repmd->getreportnopes($unit,$division,$segment,$statspb,$statinv,$fdate,$edate)); 
		/* $data = $this->repmd->getreportkomet();
		echo '<pre>';  
		print_r($data); exit; */
        $this->load->view('modules/preport/pexport_nopes_view',$data);
	}
}