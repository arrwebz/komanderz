...
class Mreportspb extends CI_Controller
{
    ...
    public function index() {
		$data = [ 
			'drdnday' => json_encode($this->repmd->getreportspbperday('MESRA')),
			'drdnmonth' => json_encode($this->repmd->getreportspbpermonth('MESRA')),
		...
		$data['content'] = $this->load->view('modules/mreport/mreport_spb_view', $data, TRUE);
	...
	public function reportfilter() {
			...
			'fdat' => $fdate,
			'edat' => $edate,
			'drd' => $this->repmd->getreportspb($unit,$division,$segment,$statspb,$payment,$fdate,$edate),
			'title' => 'MESRA',
			'brand' => 'Laporan SPB',
			'tnk' => $this->repmd->countnopeskomet(),
			'tpk' => $this->repmd->countprpokomet(),
		...
		$data['content'] = $this->load->view('modules/mreport/mfilter_spb_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function exportfilter(){
		...
		$data = array( 'title' => 'Laporan SPB Mesra : '.$strftitle,
                'drd' => $this->repmd->getreportspb($unit,$division,$segment,$statspb,$payment,$fdate,$edate)); 
		/* $data = $this->repmd->getreportkomet();
		echo '<pre>';  
		print_r($data); exit; */
           $this->load->view('modules/mreport/mexport_spb_view',$data);
	}	 
