...
class Pprpo extends CI_Controller
{
	//kode arsip
    public $intorderid;
	public $intarcoid;
    ...
    public function index() {
		$data = [ 
			'drd' => $this->ordermd->getallprpopadi(),
		...
		$data['content'] = $this->load->view('modules/pprpo/pprpo_view', $data, TRUE);
	...
	public function preview() {
		...
		// Get output html
        $html = $this->load->view('modules/pprpo/detail_pprint_view', $data, true);
	...
	public function addprpo() {
		if (isset($_POST['cmdsave'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
				...
				$month = mdate('%m', $time);
				$year = mdate('%y', $time);
				$strformat = $intnomor ."/".$strordertype."/P". /* $strunit[0]. */$strjobtype . "/". $month . "/" . $year;
		...
		$data['content'] = $this->load->view('modules/pprpo/add_pprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function editprpo() {
		...
		$data['content'] = $this->load->view('modules/pprpo/edit_pprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function createinvoice() {
		...
		$data['content'] = $this->load->view('modules/pprpo/invoice_pprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function addspb() {
		...
		$data['content'] = $this->load->view('modules/pprpo/add_pspb_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function addtermininv() {
		...
		$data = [ 
			...
			'prpoid' => $prpoid,
            'drd' => $this->ordermd->getalltermininvpadi($division,$segmen),
			'tnk' => $this->repmd->countnopeskomet(),
			'tpk' => $this->repmd->countprpokomet(),
			'tsk' => $this->repmd->countspbkomet(),
			'tnm' => $this->repmd->countnopesmesra(),
		...
		$data['content'] = $this->load->view('modules/pprpo/termin_pprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function createtermin()
    {
		if ($this->input->post()) {
			...
			redirect(base_url().'pprpo/details/'.$prpoid);
			}
		}
    }
