...
class Request extends CI_Controller
{
	...
	private function bindskins(){
        $css_head = array(
            array($this->config->item('skins_uri').'dist/css/app.css'), 
			array($this->config->item('skins_uri').'dist/plugins/pace/css/pace.min.css'),
			array($this->config->item('skins_uri').'dist/plugins/datatables.net-bs/css/dataTables.bootstrap.min.css'),
            array($this->config->item('skins_uri').'dist/plugins/font-awesome/css/font-awesome.min.css'),
            array('https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,600,700,300italic,400italic,600italic'),
        );
        $js_head = array(
            array($this->config->item('skins_uri').'dist/plugins/jquery/dist/jquery.min.js'),
			array($this->config->item('skins_uri').'dist/plugins/jquery-ui/jquery-ui.min.js'),
        ); 
        $js_content  = array(
            array($this->config->item('skins_uri').'dist/js/app.js'),
            array($this->config->item('skins_uri').'dist/plugins/datatables.net/js/jquery.dataTables.min.js'),
            array($this->config->item('skins_uri').'dist/plugins/datatables.net-bs/js/dataTables.bootstrap.min.js')
        );
    ...
    public function index() {
		$data = [ 
			'drd' => $this->ordermd->getallnopeskomet(),
		...
		$data['content'] = $this->load->view('modules/request/req_list_view', $data, TRUE);
	...
	public function addnopes() {
		...
		$data['content'] = $this->load->view('modules/knopes/add_knopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
