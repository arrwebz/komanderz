...
class Pnopes extends CI_Controller
{
	//kode arsip
    public $intorderid;
	public $intarcoid;
    ...
    public function index() {
		$data = [ 
			'drd' => $this->ordermd->getallnopespadi(),
		...
		$data['content'] = $this->load->view('modules/pnopes/pnopes_view', $data, TRUE);
	...
	public function search() {
		...
		$data['content'] = $this->load->view('modules/pnopes/search_pnopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
    }
	...
	public function details() {
		...
		$data['content'] = $this->load->view('modules/pnopes/detail_pnopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function preview() {
		...
		// Get output html
        $html = $this->load->view('modules/pnopes/detail_pprint_view', $data, true);
	...
	public function addnopes() {
		if (isset($_POST['cmdsave'])) { 
            ...
            if ($this->form_validation->run() == TRUE) {
				...
				$this->ordermd->strstatorder = 'PADI';
				...
				$this->ordermd->intfaknum = '7619'.$this->input->post('txtFaknum');
				$fp = $this->ordermd->getFakturPajak('KOMET'); /* PADI PAKAI FAKTUR KOMET */
				$this->ordermd->intfaknum = $fp[0]['code'].$this->input->post('txtFaknum');
				/* $faknum = $this->input->post('txtFaknum');
				if($this->input->post('optUnit') == 'PADI'){
		...
		$data['content'] = $this->load->view('modules/pnopes/add_pnopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}	
	...
	if (isset($_POST['cmdsave'])) {
		$error = [
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                /* UPLOAD FAKTUR */
                if (!empty($_FILES['txtFile']['name'])) {
                    $photo_path = $this->config->item('uploads_dir') . 'fakturpajak/padi/' .  $this->input->post('hdnOrderid');
                    if (!file_exists($photo_path)) {
                        mkdir($photo_path, 0777, true);
                    }
                    $title = $this->input->post('txtKodenomor');
                    ...
                    if ($this->upload->do_upload('txtFile')) {
                        ...
                        $this->ordermd->strfiles = $upload_data['file_name'];
                    }
                }
                /* END UPLOAD FAKTUR */
		...
		$data['content'] = $this->load->view('modules/pnopes/edit_pnopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function addspb() {
		...
		$data['content'] = $this->load->view('modules/pnopes/add_pspb_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
