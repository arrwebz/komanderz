...
class Mfiling extends CI_Controller
{
	public $intspbid;
	public $intorderid;
	public $strcode;
    ...
    public function index() {
		...
		$data = [ 
			'drd' => $this->spbmd->getmyfiling($this->session->userdata('userid'),'MESRA'),
			'drp' => $this->spbmd->getpfiling('MESRA'),
		...
		$data['content'] = $this->load->view('modules/mfiling/mfiling_view', $data, TRUE);
	...
	public function addnew() {
		...
		$data = [ 
			...
			'tpm' => $this->repmd->countprpomesra(),
			'tsm' => $this->repmd->countspbmesra(),
			'drd' => $this->spbmd->getmyspbbyinvoicetoday('MESRA'),
			'userid' => $this->session->userdata('userid'),
			'role' => $this->session->userdata('role'),
			'group' => $this->session->userdata('group'),
			'fullname' => $this->session->userdata('userfullname'),
		...
		$data['content'] = $this->load->view('modules/mfiling/mfiling_add_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
		...
		if ($this->input->post()) {
			...
			redirect(base_url().$this->router->fetch_class());
				...
				foreach($_POST['aprv'] as $spbidaprv){ 
					...
					$this->spbmd->strcodeaprv = 'RAPM'.date('Ymdhis');
	...
	public function myreqspb() {
		...
		$data['content'] = $this->load->view('modules/mfiling/mfiling_today_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function appspb() {
		...
		$data['content'] = $this->load->view('modules/mfiling/mfiling_app_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
