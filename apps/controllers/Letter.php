...
class Letter extends CI_Controller
{
    ...
    private function bindskins(){
        $css_head = array(
            ...
            array($this->config->item('skins_uri').'libs/select2/dist/css/select2.41.min.css'),
        ...
        $js_content  = array(
            ...
            array($this->config->item('skins_uri').'libs/select2/dist/js/select2.41.min.js'),
    ...
    public function index() {
        $data = [
            'drd' => $this->ltmd->getallletter(array()),
            'title' => 'KOMET ',
            'brand' => 'Daftar Surat',
        ...
        $data['content'] = $this->load->view('modules/letter/letter_view', $data, TRUE);
    ...
    function listdata(){
        ...
        if($post){
            ...
            $view = $this->load->view('modules/letter/listdata_view', $data, TRUE);
    ...
    function ajaxadd(){
        ...
        if($post){
            ...
            $this->ltmd->strcode = $number.'/'.$post['type'].'/'.$post['initial'].'/'.$post['unit'].'/'.month_to_romawi_letter($post['month']).'/'.substr($post['year'], -2);
            //$this->ltmd->strcode = $post['code'];
            $this->ltmd->strtype = $post['type'];
            $this->ltmd->strinitial = $post['initial'];
            $this->ltmd->strsubject = $post['subject'];
    ...
    function ajaxdetail(){
        ...
        if($post){
            ...
            if(empty($detail)){
                ...
                $view = $this->load->view('modules/letter/editletter_view', $data, TRUE);
    ...
    function ajaxedit(){
        ...
        if($post){
            ...
            if(count($ex_code_old) > 0){
                ...
                $this->ltmd->strcode = $ex_code_old[0].'/'.$post['type'].'/'.$post['initial'].'/'.$post['unit'].'/'.month_to_romawi_letter($post['month']).'/'.substr($post['year'], -2);
                //$this->ltmd->strcode = $post['code'];
                $this->ltmd->strtype = $post['type'];
                $this->ltmd->strinitial = $post['initial'];
                $this->ltmd->strsubject = $post['subject'];
    ...
    function nomorsurat(){
        $get_int = $this->ltmd->getlastnumber('INT');
        if(count($get_int) > 0){
        ...
        $get_ext = $this->ltmd->getlastnumber('EXT');
        if(count($get_ext) > 0){
