...
class Mreportnps extends CI_Controller
{
    ...
    public function index() {
		$data = [ 
			'drdnday' => json_encode($this->repmd->getreportnopesperday('MESRA')),
			'drdnmonth' => json_encode($this->repmd->getreportnopespermonth('MESRA')),
		...
		$data['content'] = $this->load->view('modules/mreport/mreport_nopes_view', $data, TRUE);
	...
	public function reportfilter() {
			...
			'fdat' => $fdate,
			'edat' => $edate,
			'drd' => $this->repmd->getreportnopes($unit,$division,$segment,$statspb,$statinv,$fdate,$edate),
			'title' => 'MESRA',
			'brand' => 'Laporan NOPES',
			'tnk' => $this->repmd->countnopeskomet(),
			'tpk' => $this->repmd->countprpokomet(),
		...
		$data['content'] = $this->load->view('modules/mreport/mfilter_nopes_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function exportfilter(){
		...
		$strftitle = $segment.'/'.$fdate.'/'.$edate.'/MESRA';
		$data = array( 'title' => 'Laporan Invoice Mesra : '.$strftitle,
                'drd' => $this->repmd->getreportnopes($unit,$division,$segment,$statspb,$statinv,$fdate,$edate)); 
		/* $data = $this->repmd->getreportkomet();
		echo '<pre>';  
		print_r($data); exit; */
           $this->load->view('modules/mreport/mexport_nopes_view',$data);
	}	 
