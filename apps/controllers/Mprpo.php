...
class Mprpo extends CI_Controller
{
	//kode arsip
    public $intorderid;
	public $intarcoid;
    ...
    public function index() {
		$data = [ 
			'drd' => $this->ordermd->getallprpomesra(),
		...
		$data['content'] = $this->load->view('modules/mprpo/mprpo_view', $data, TRUE);
	...
	public function details() {
		...
		$data['content'] = $this->load->view('modules/mprpo/detail_mprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function preview() {
        ...
        //Load html view
	    //$this->html2pdf->html($this->load->view('modules/mprpo/detail_mprint_view', $data, true));
		...
		// Get output html
        $html = $this->load->view('modules/mprpo/detail_mprint_view', $data, true);
	...
	public function addprpo() {
		...
		$data['content'] = $this->load->view('modules/mprpo/add_mprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function editprpo() {
		...
		$data['content'] = $this->load->view('modules/mprpo/edit_mprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function createinvoice() {
		if (isset($_POST['cmdsave'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
				...
				$this->ordermd->intfaknum = '0863'.$this->input->post('txtNomorfak');
		...
		$data = [
			...
			'spbid' => $this->intspbid,
            'statusorder' => $this->strstatorder,
			'kodenomor' => str_replace('PRPO','ODR',$this->strcode),
			'unit' => $this->strunit,
		...
		$data['content'] = $this->load->view('modules/mprpo/invoice_mprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function addspb() {
		...
		$data['content'] = $this->load->view('modules/mprpo/add_mspb_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	...
	public function addtermininv() {
		...
		$data = [ 
			...
			'prpoid' => $prpoid,
			'drd' => $this->ordermd->getalltermininvmesra($division,$segmen),
			'tnk' => $this->repmd->countnopeskomet(),
			'tpk' => $this->repmd->countprpokomet(),
			'tsk' => $this->repmd->countspbkomet(),
			'tnm' => $this->repmd->countnopesmesra(),
		...
		$data['content'] = $this->load->view('modules/mprpo/termin_mprpo_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function createtermin()
    {
		if ($this->input->post()) {
			...
			redirect(base_url().'mprpo/details/'.$prpoid);
			}
		}
    }
