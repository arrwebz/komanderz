...
class Targetsales extends CI_Controller{
    ...
    public function __construct()
    {
		...
		$this->load->model('targetsales_model', 'tsmd');
    }
	...
	public function index() {
		...
		$data = [
            'drd' => $this->tsmd->getalltargetsales(),
            'title' => 'Target Sales',
			'brand' => '',
		...
		$data['content'] = $this->load->view('modules/targetsales/targetsales_view', $data, TRUE);
    ...
    public function search() {
        if($_POST){
            ...
            $data['content'] = $this->load->view('modules/targetsales/search_targetsales_view', $data, TRUE);
            $this->load->view('templates/base', $data, FALSE);
        }
    }
	...
	public function addtargetsales() {
		...
		$data['content'] = $this->load->view('modules/targetsales/add_targetsales_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}
	...
	public function edittargetsales() {
		if (isset($_POST['cmdsave'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->tsmd->intidtargetsales = $this->input->post('txtIdtargetsales');
                $this->tsmd->strnama = $this->input->post('txtNama');
                $this->tsmd->strnilai = $this->input->post('txtNilai');
                $this->tsmd->strtipe = $this->input->post('optTipe');
                $this->tsmd->strbulan = $this->input->post('optBulan');
				...
				$this->tsmd->edittargetsales();
                redirect(base_url($this->router->fetch_class()));
            }
        }

        $drd = $this->tsmd->getsingletargetsales($this->uri->segment(3));
        if (count($drd) > 0) {
            foreach ($drd as $row) {
                $this->intidtargetsales = $row['idtargetsales'];
		...
		$data = [
            'idtargetsales' => $this->intidtargetsales,
            'nama' => $this->strnama,
            'nilai' => $this->strnilai,
            'tipe' => $this->strtipe,
            'bulan' => $this->strbulan,
		...
		$data['content'] = $this->load->view('modules/targetsales/edit_targetsales_view', $data, TRUE);
        $this->load->view('templates/base', $data, FALSE);
	}

    function delete(){
        if ($this->input->post()) {
            $this->tsmd->delete($_POST['idtargetsales']);
            echo "success";
        }
    }
}	